...
export const tools: Tool[] = [
  // ── VM lifecycle + snapshots ────────────────────────────────────────────────
  {
    ...
    actions: [
      { id: "list", label: "List VMs", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh list --all" },
      { id: "start", label: "Start", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh start {vmName} && sudo virsh list --all" },
      { id: "shutdown", label: "Shutdown", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh shutdown {vmName} && sudo virsh list --all" },
      { id: "suspend", label: "Suspend", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh suspend {vmName} && sudo virsh list --all" },
      { id: "resume", label: "Resume", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh resume {vmName} && sudo virsh list --all" },
      { id: "reboot", label: "Reboot", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh reboot {vmName} && sudo virsh list --all" },
      { id: "reset", label: "Force reset", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh reset {vmName} && sudo virsh list --all" },
      { id: "destroy", label: "Force stop (destroy)", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh destroy {vmName} && sudo virsh list --all" },
      { id: "undefine", label: "Delete (destroy + undefine)", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh destroy {vmName} ; sudo virsh undefine {vmName} && sudo virsh list --all" },
      { id: "viewer", label: "Open viewer", mode: "terminal", sudo: true, section: "manage", command: "sudo virt-viewer {vmName}" },
      { id: "snap-list", label: "List snapshots", mode: "terminal", sudo: true, section: "snapshots", command: "sudo virsh snapshot-list {vmName}" },
      { id: "snap-create", label: "Create snapshot", mode: "terminal", sudo: true, section: "snapshots", command: `sudo virsh snapshot-create-as {vmName} '{snapName}' "{snapDesc}" && sudo virsh snapshot-list {vmName}` },
      { id: "snap-revert", label: "Revert snapshot", mode: "terminal", sudo: true, section: "snapshots", command: "sudo virsh snapshot-revert {vmName} '{snapName}' && sudo virsh snapshot-list {vmName}" },
      { id: "snap-delete", label: "Delete snapshot", mode: "terminal", sudo: true, section: "snapshots", command: "sudo virsh snapshot-delete {vmName} '{snapName}' && sudo virsh snapshot-list {vmName}" },
    ],
  },
  ...
  // ── images: download an image, create a VM from ISO, import an existing disk ──
  {
    ...
    fields: [
      ...
      { id: "cDiskPath", type: "text", label: "Disk path (created)", section: "create", default: "/var/lib/libvirt/images/{cVmName}.qcow2" },
      { id: "cDiskSize", type: "text", label: "Disk size (GB)", section: "create", default: "64" },
      { id: "cNet", type: "text", label: "Bridge / network interface", section: "create", default: "virbr0" },
      {
        id: "cVirtioIso",
        type: "text",
      ...
      { id: "iDisk", type: "file", fileKind: "file", label: "Disk image (qcow2)", section: "import", help: "An existing bootable disk to import (under /var/lib/libvirt/images/)." },
      ...
      { id: "iNet", type: "text", label: "Bridge / network interface", section: "import", default: "virbr0" },
    ],
    actions: [
      {
        id: "download",
        ...
        section: "download",
        command:
          "cd /var/lib/libvirt/images && sudo wget -N {dlUrl} && f=$(basename {dlUrl}) && case \"$f\" in *.7z) sudo 7z x -y \"$f\" ;; *.zip) sudo unzip -o \"$f\" ;; *.tar.*) sudo tar xf \"$f\" ;; esac ; sudo chmod 644 /var/lib/libvirt/images/*.qcow2 /var/lib/libvirt/images/*.iso 2>/dev/null ; ls -lh /var/lib/libvirt/images",
      },
      {
        id: "create",
      ...
      },
      {
        ...
        section: "import",
        command:
          "sudo virt-install --os-variant {iOsVariant} --name {iVmName} --ram {iRam} --vcpus {iVcpus} --cpu host --disk path={iDisk},bus=virtio,cache=writeback --network bridge={iNet},model=virtio --graphics spice,listen=0.0.0.0 --video qxl --channel spicevmc,target_type=virtio,name=com.redhat.spice.0 --console pty,target_type=serial --import",
      },
    ],
  },
  ...
  // ── libvirt networks + IP forwarding ────────────────────────────────────────
  {
    ...
    actions: [
      { id: "net-list", label: "List networks", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh net-list --all" },
      { id: "net-info", label: "Network info", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh net-info {netName}" },
      { id: "net-start", label: "Start network", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh net-start {netName} && sudo virsh net-list --all" },
      { id: "net-stop", label: "Stop network", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh net-destroy {netName} && sudo virsh net-list --all" },
      { id: "net-destroy", label: "Destroy (undefine + firewall cleanup)", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh net-destroy {netName} ; sudo virsh net-undefine {netName} ; sudo iptables -D FORWARD -i virbr-{netName} -j ACCEPT 2>/dev/null ; sudo iptables -D FORWARD -o virbr-{netName} -j ACCEPT 2>/dev/null ; sudo virsh net-list --all" },
      { id: "net-leases", label: "List DHCP leases", mode: "terminal", sudo: true, section: "manage", command: "sudo virsh net-dhcp-leases {netName}" },
      ...
      },
      {
        ...
        command:
          `echo "<network><name>{cnetName}</name><bridge name='virbr-{cnetName}' stp='on' delay='0'/><ip address='{cnetIp}' netmask='{cnetMask}'><dhcp><range start='{cnetDhcpStart}' end='{cnetDhcpEnd}'/></dhcp></ip></network>" | sudo virsh net-define /dev/stdin && sudo virsh net-autostart {cnetName} && sudo virsh net-start {cnetName} && sudo virsh net-list --all`,
      },
      {
        id: "create-isolated",
